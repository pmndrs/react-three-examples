 ...
 *   layers 0/1/2 (declarative fiber `layers-mask` dash-path prop on each mesh AND on
 *   the camera), leva checkboxes composing the camera's mask bit-by-bit — same
 *   compiled scene, the camera simply stops drawing a layer
 ...
 *   the original's stateless `camera.layers.toggle(n)` callbacks become a mask composed
 *   from the three checkbox values and set declaratively via `layers-mask` on the
 *   camera (direct value controls beat toggles that hide state; identical behavior).
 *   The Inspector addon itself is dropped (this repo's shell has no inspector wiring)
