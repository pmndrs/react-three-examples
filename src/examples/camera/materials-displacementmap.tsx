 ...
 * - A point light attached to the camera as a fill "headlight", declaratively:
 *   `<OrthographicCamera makeDefault><pointLight/></OrthographicCamera>`
 *
...
function NinjaHead() {
  const { metalness, roughness, aoMapIntensity, envMapIntensity, displacementScale, normalScale } = useControls(
    'materials-displacementmap',
    {
      material: folder({
...
export default function MaterialsDisplacementmap() {
  ...
  return (
    <Canvas renderer>
      <OrthographicCamera makeDefault position={[0, 0, 1500]} near={1} far={10000}>
        {/* Declarative headlight: the child light is the JSX form of camera.add(light). */}
        <pointLight color="#ff6666" intensity={3} distance={0} decay={0} />
      </OrthographicCamera>
      <OrthographicFraming />
