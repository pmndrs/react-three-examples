 ...
 *   step via `renderer.compute()` and mutating the sphere-position uniform per
 *   STEP, not per frame
 ...
 *   the sim component (Cloth.tsx), ordered after its creator hooks: custom-node
 *   materials in this unlit scene must see `scene.environment` on their first
 *   shader build (AGENTS.md B15) while creator hooks must precede the suspension
 *   (B18) — the original simply awaits the loader before building anything
