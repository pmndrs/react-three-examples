...
export function Furniture() {
  // REVIEW(shared-instance): three materials shared across ~20 meshes (pedestals, chair
  // frame, slats). JSX children would create one material per mesh, so the shared
  // instance is the right performance call — but the repetition here is a hint this
  // scene may want <Instances> for the pedestals and slats instead.
  ...
  return (
    <>
      ...
      <mesh position={[-5, -1.6, -2]} scale={1.8}>
        <latheGeometry args={[VASE_PROFILE, 24]} />
      ...
      <group scale={1.76} position={[4, 0.948, -2.5]} rotation-y={-0.6}>
        <mesh position={[0, -1.35, 0]} material={fabricMaterial}>
          <roundedBoxGeometry args={[0.9, 0.25, 0.8, 4, 0.06]} />
        </mesh>
        <mesh position={[0, -0.95, -0.4]} rotation-x={-0.3} material={fabricMaterial}>
          <roundedBoxGeometry args={[0.9, 0.6, 0.12, 4, 0.04]} />
        </mesh>
