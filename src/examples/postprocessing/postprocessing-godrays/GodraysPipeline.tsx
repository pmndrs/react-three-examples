...
export function GodraysPipeline({ light }: GodraysPipelineProps) {
  const { raymarchSteps, density, maxDensity, distanceAttenuation, edgeRadius, edgeStrength, blur } = useControls(
    'Godrays',
    {
  ...
  const { renderPipeline, passes } = useRenderPipeline(({ renderPipeline, passes, camera }) => {
    ...
    // raymarchSteps is a uint()-typed field on godraysPass itself (pattern b would
    // swap it for a float-typed fiber uniform and lose that typing) — kept as a
    // direct `.value` mutation below for both it and the density/attenuation knobs.
