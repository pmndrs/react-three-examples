...
// render-takeover rule this is the intended case (cousin: `camera/CameraRig.tsx`).
...
export function InsetView() {
  ...
  useFrame(
    (state) => {
      ...
      // same cast as backdrop-area.tsx).
      ...
      // NOTE on y: WebGPURenderer's setViewport/setScissor y is TOP-origin (the common
      // Renderer passes it straight to setScissorRect, no WebGL-style bottom-origin
      // flip — verified in src/renderers/webgpu/WebGPUBackend.js). The original keeps
      // the webgl version's bottom-origin math and so its inset silently lands
      // BOTTOM-left on WebGPU; we pin the pane top-left deliberately (divergence —
      // clears this shell's bottom-left titleblock overlay).
