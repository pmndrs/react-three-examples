...
- **Phases** (SPEC §4) are _what gets ported_: **Phase 1** = the `webgpu_`* set;
  **Patterns track** = 12–20 app-scale examples; **Phase 2** = curated webgl-only
  examples + the loader gallery; **Final phase** = WebXR (webaudio was ported and the TSL
  tooling pages excluded as internal, both 2026-09-03).
...
- **Final phase**: the 3 deferred (`xr-cubes`, `xr-native-layers`, `xr-rollercoaster`) plus
  the 26 `webxr_*` originals — WebXR needs `@react-three/xr` on v10, and today even the
  `webgpu_xr_*` originals run their sessions through a WebGL fallback renderer swap
  (`setupWebGLXRFallback`) that fiber cannot express. `compute-audio` shipped with the
  webaudio wave (2026-09-03). `tsl-editor`, `tsl-graph`,
